...
- **BRAIN_CONTEXT**: nixos
- **BRAIN_REPO**: nix-services
- **Purpose**: Shared service layer of the homelab — reusable NixOS service
  modules, Docker Compose definitions, systemd supervision, shared options,
  service-side operational docs, and runtime-secret helpers.
- **Private counterpart**: `../nix-services-private/` (never pushed to GitHub)
...
- Only content explicitly published via `brainctl publish` goes here
- Raw investigations live in `../nix-services-private/.brain/` — BRAIN_ROOT points there
