...
```yaml
  - job_name: "loki"
    static_configs:
      - targets: ["loki.hhlab.home.arpa:3100"]
  ...
  - name: loki
    rules:
      ...
      - alert: LokiDiskFillingUp
        expr: |
          (
            node_filesystem_avail_bytes{instance="rpi-box-03.hhlab.home.arpa:9100",mountpoint="/srv/loki",fstype!~"tmpfs|overlay"}
            /
            node_filesystem_size_bytes{instance="rpi-box-03.hhlab.home.arpa:9100",mountpoint="/srv/loki",fstype!~"tmpfs|overlay"}
          ) < 0.15
        for: 15m
        labels:
          severity: warning
