{
  ...
  inherit
    ...
    synologySnmpOperationsDashboardJson
    githubProfileOperationsDashboardJson
    applicationsReliabilityDashboardJson
    appInternalsDashboardJson
    homeAssistantOperationsDashboardJson
    unifiOverviewDashboardJson
    kubernetesOverviewDashboardJson
    fleetResourcesDashboardJson
    ;
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc."${serviceName}/provisioning/dashboards/synology-snmp-operations.json" = lib.mkIf (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) {
      text = synologySnmpOperationsDashboardJson;
      mode = "0444";
    };
    environment.etc."${serviceName}/provisioning/dashboards/github-profile-operations.json" = lib.mkIf (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) {
      text = githubProfileOperationsDashboardJson;
      mode = "0444";
    };
    environment.etc."${serviceName}/provisioning/dashboards/applications-reliability.json" = lib.mkIf (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) {
      text = applicationsReliabilityDashboardJson;
      mode = "0444";
    };
    environment.etc."${serviceName}/provisioning/dashboards/app-internals.json" = lib.mkIf (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) {
      text = appInternalsDashboardJson;
      mode = "0444";
    };
    environment.etc."${serviceName}/provisioning/dashboards/home-assistant-operations.json" = lib.mkIf (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) {
      text = homeAssistantOperationsDashboardJson;
      mode = "0444";
    };
    ...
    environment.etc."${serviceName}/provisioning/dashboards/fleet-resources.json" = lib.mkIf (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) {
      text = fleetResourcesDashboardJson;
      mode = "0444";
    };
    ...
    systemd.services.${serviceName} = {
      ...
      restartTriggers =
        ...
        ++ lib.optionals (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) [
          ...
          config.environment.etc."${serviceName}/provisioning/dashboards/synology-snmp-operations.json".source
          config.environment.etc."${serviceName}/provisioning/dashboards/github-profile-operations.json".source
          config.environment.etc."${serviceName}/provisioning/dashboards/applications-reliability.json".source
          ...
          config.environment.etc."${serviceName}/provisioning/dashboards/fleet-resources.json".source
        ]
      ...
      serviceConfig = {
        ...
        ExecStartPre =
          ...
          ++ lib.optionals (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter) [
            ...
            "${pkgs.runtimeShell} -c 'test -s ${composeDir}/provisioning/dashboards/synology-snmp-operations.json'"
            "${pkgs.runtimeShell} -c 'test -s ${composeDir}/provisioning/dashboards/github-profile-operations.json'"
            "${pkgs.runtimeShell} -c 'test -s ${composeDir}/provisioning/dashboards/applications-reliability.json'"
            ...
            "${pkgs.runtimeShell} -c 'test -s ${composeDir}/provisioning/dashboards/fleet-resources.json'"
          ]
          ++ lib.optionals (cfg.provisioning.enable && cfg.provisioning.dashboards.enableStarter && cfg.provisioning.datasources.loki.url != null) [
