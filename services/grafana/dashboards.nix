  ...
  applicationsReliabilityDashboardJson = builtins.toJSON {
    ...
    panels = [
      ...
      }
      {
        ...
        targets = [
          {
            expr = "sum(sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker|fossflow@docker|searxng@docker\",code=~\"5..\"}[15m])) > bool 0) or vector(0)";
            refId = "A";
          }
        ];
      }
      {
        ...
        targets = [
          {
            expr = "100 * (1 - ((sum(rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker|fossflow@docker|searxng@docker\",code=~\"5..\"}[5m])) or vector(0)) / clamp_min((sum(rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker|fossflow@docker|searxng@docker\"}[5m])) or vector(0)), 0.001)))";
            refId = "A";
          }
        ];
      }
      ...
      }
      {
        ...
        targets = [
          {
            expr = "sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker|fossflow@docker|searxng@docker\"}[5m]))";
            legendFormat = "{{service}}";
            refId = "A";
          }
        ];
      }
      {
        ...
        targets = [
          {
            expr = "100 * (sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker|fossflow@docker|searxng@docker\",code=~\"5..\"}[5m])) / clamp_min(sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker|fossflow@docker|searxng@docker\"}[5m])), 0.001))";
            legendFormat = "{{service}}";
            refId = "A";
          }
        ];
      ...
      }
      {
        ...
        targets = [
          {
            expr = "max by (container_label_com_docker_compose_service) ((time() - container_last_seen{job=\"cadvisor\",container_label_com_docker_compose_service=~\"authentik-server|authentik-worker|vikunja|timetagger|homepage|uptime-kuma|diagrams-net|excalidraw|owntracks-recorder|traggo|dozzle|fossflow|searxng\"}) < bool 180)";
