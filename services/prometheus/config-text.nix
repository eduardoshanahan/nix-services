{
  ...
  alertRulesText =
    lib.concatStringsSep "\n" [
      ...
      "        expr: max by (container_label_com_docker_compose_service) ((time() - container_last_seen{job=\"cadvisor\",container_label_com_docker_compose_service=~\"authentik-server|authentik-worker|vikunja|smtp-relay|timetagger|homepage|uptime-kuma\"}) < bool 180) == 0"
      "        for: 5m"
      "        labels:"
      "          severity: critical"
      "        annotations:"
