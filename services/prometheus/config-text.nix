{
  ...
  alertRulesText =
    lib.concatStringsSep "\n" [
      ...
      "        expr: (100 * (sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker\",code=~\"5..\"}[5m])) / clamp_min(sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker\"}[5m])), 0.01))) > 5 and on(service) (sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker\"}[5m])) > 0.02)"
      "        for: 10m"
      "        labels:"
      "          severity: warning"
      "        annotations:"
      ...
      "        expr: histogram_quantile(0.95, sum by (service, le) (rate(traefik_service_request_duration_seconds_bucket{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker\"}[5m]))) > 2 and on(service) (sum by (service) (rate(traefik_service_requests_total{service=~\"authentik@docker|vikunja@docker|timetagger@docker|homepage@docker|ghost-blog@docker|uptime-kuma@docker|diagrams-net@docker|excalidraw@docker|owntracks-recorder@docker|traggo@docker|dozzle@docker\"}[5m])) > 0.02)"
      "        for: 15m"
      "        labels:"
      "          severity: warning"
      "        annotations:"
