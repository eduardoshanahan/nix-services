...
first shared HTTP ingress service, explicitly supporting **deployment and
...
- Deploy Traefik as the **first routed service** on each box
...
The Traefik module is option-driven and currently spans these source files:
...
The checked-in `docker-compose.yml` is a reference snapshot. The active runtime
compose is generated by the module and written to `/etc/traefik`.
...
## 5. Compose and Runtime Rules for Traefik

The generated Traefik compose/runtime configuration must:
...
- Assume Docker is available from the consumer host configuration
...
Consumer hosts MUST enable Traefik **only by importing the exported service
module**.
...
Hosts may provide minimal non-secret option values, but they should not copy or
redefine Traefik internals.
...
5. Consumer host imports Traefik service
