...
```sshconfig
Host gitea-hhlab
  HostName gitea.hhlab.home.arpa
  Port 2222
  User git
...
- [ ] Login to registry:
  - `docker login gitea.hhlab.home.arpa`
- [ ] Push a test image:
  - `docker tag alpine:latest gitea.hhlab.home.arpa/<owner>/alpine-test:latest`
  - `docker push gitea.hhlab.home.arpa/<owner>/alpine-test:latest`
...
- [ ] Compose status on NAS:
  - `cd /volume1/docker/homelab/hhnas4/gitea && sudo /usr/local/bin/docker compose ps`
...
# Clone
git clone ssh://git@gitea.hhlab.home.arpa:2222/<owner>/<repo>.git

# Existing repo remote
git remote set-url origin ssh://git@gitea.hhlab.home.arpa:2222/<owner>/<repo>.git
```
...
# Pull back
docker pull gitea.hhlab.home.arpa/<owner>/alpine-test:latest
```
...
```bash
# Public HTTPS path
curl -skI https://gitea.hhlab.home.arpa/ | head -n 5

# SSH endpoint reachability
ssh -o BatchMode=yes -o ConnectTimeout=8 -p 2222 -T git@gitea.hhlab.home.arpa
...
ssh hhnas4 "curl -sSI http://127.0.0.1:9080/ready | head -n 5"
ssh hhnas4 "cd /volume1/docker/homelab/hhnas4/promtail && sudo /usr/local/bin/docker compose ps"
```
