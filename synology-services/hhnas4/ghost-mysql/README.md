...
Dedicated MySQL 8 stack for Ghost production use on `hhnas4`.

## Purpose

- Runs a dedicated MySQL 8 server for Ghost.
- Keeps database state in a Docker-managed named volume on this Synology host.
- Can be validated independently before Ghost is deployed on `rpi-box-02`.
...
```text
/volume1/docker/homelab/hhnas4/ghost-mysql
```
...
```bash
cd synology-services/hhnas4/ghost-mysql
...
```bash
./deploy.sh hhnas4.internal.example /volume1/docker/homelab/hhnas4/ghost-mysql
```
...
- This stack uses a named volume intentionally. The original bind-mount
  approach failed on `hhnas4` because Synology ACLs blocked MySQL's internal
  non-root writes during initialization.

## Validation goals before Ghost

- Container reports `healthy`.
- Port `3306` is reachable from `rpi-box-02`.
