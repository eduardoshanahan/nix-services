...
```bash
cd synology-services/hhnas4/archivebox
...
- The default template binds ArchiveBox to `127.0.0.1` and sets
  `ARCHIVEBOX_ALLOWED_HOSTS=archivebox.<homelab-domain>`, which is the intended
...
- Client URL: `https://archivebox.<homelab-domain>/`
...
- `curl -sSI -H 'Host: archivebox.<homelab-domain>' http://127.0.0.1:8000/`
  returns a redirect or page response.
- `curl -skI https://archivebox.<homelab-domain>/` returns a redirect or page
