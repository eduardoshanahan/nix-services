...
2. From LAN, verify `http://hhnas4.<homelab-domain>:9100/metrics` responds.
3. In Prometheus UI, confirm target `hhnas4.<homelab-domain>:9100` is `UP` under job `synology-nodes`.
...
2. Create a sending rule with:
   - Protocol: `TCP`
   - Destination server: `192.168.1.10` (or `loki.<homelab-domain>`)
